...
type Resposta = Resposta10 | RespostaOutros;
    ...
    <View style={styles.regrasCard}>
      ...
      <View style={styles.regraItem}>
        ...
        <Text style={styles.regraText}>
          Pode avançar com <Text style={styles.regraBold}>até 2 negativas</Text>,
          desde que cada negativa tenha no mínimo <Text style={styles.regraBold}>7 valores</Text>.
      ...
      <View style={styles.regraItem}>
        ...
        <Text style={styles.regraText}>
          Qualquer disciplina com média <Text style={styles.regraBold}>abaixo de 7 valores</Text>{' '}
          ou <Text style={styles.regraBold}>mais de 2 negativas</Text> → Reprova directamente.
        </Text>
      </View>
      <View style={styles.regraItem}>
        ...
        <Text style={styles.regraText}>
          Disciplina <Text style={styles.regraBold}>terminal</Text> com negativa →{' '}
...
export default function PromocaoStatusModal({ visible, onClose, alunoId, alunoNome }: Props) {
  ...
  const dataOther = !is10 && data ? (data as RespostaOutros) : null;

  const geralCfg = data10 ? geralConfig(data10.situacaoGeral) : null;
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          {!loading && !error && data && (
            <>
              ...
              {/* Situação Geral — apenas para 10ª */}
              {data10 && geralCfg && (
                <View style={[styles.geralBanner, { backgroundColor: geralCfg.bg, borderColor: geralCfg.border }]}>
                  <Ionicons name={geralCfg.icon} size={22} color={geralCfg.color} />
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    <Text style={[styles.geralLabel, { color: geralCfg.color }]}>{geralCfg.label}</Text>
                    {data10.motivoGeral ? (
                      <Text style={[styles.geralMotivo, { color: geralCfg.color }]}>{data10.motivoGeral}</Text>
                    ) : null}
                  </View>
                </View>
              )}
              ...
              {/* Estatísticas rápidas — 10ª classe */}
              {data10 && (
                <View style={styles.statsRow}>
                  {[
                    {
                      label: 'Negativas',
                      val: data10.disciplinas.filter(d => d.mediaAnual < data10.notaMin).length,
                      color: Colors.danger,
                    },
                    {
                      label: 'Máx. Permitido',
                      val: data10.maxNegativasPermitidas,
                      color: Colors.textMuted,
                    },
                    {
                      label: 'Com Exame',
                      val: data10.disciplinas.filter(d => d.situacao === 'negativa_exame_terminal').length,
                ...
                <>
                  ...
                  {data10
                    ? data10.disciplinas
                        ...
                        .map(d => (
                          <DiscCard10 key={d.disciplina} disc={d} notaMin={data10.notaMin} />
                        ))
                    : dataOther?.disciplinas
                        .sort((a, b) => a.mediaAnual - b.mediaAnual)
                        .map(d => (
                          <DiscCardSimples key={d.disciplina} disc={d} notaMin={data.notaMin} />
                        ))
                  }
                </>
              )}
